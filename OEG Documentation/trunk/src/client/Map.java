...
class Map extends JPanel
    ...
    private int               s, h, w;                                     // size
                                                                           // height
                                                                           // and
                                                                           // width
                                                                           // variables
    private int[]             xMinCords;                                   // this
                                                                           // an
                                                                           // array
                                                                           // of
                                                                           // the
    ...
    private int               squareSize;                                  // this
                                                                           ...
                                                                           // mainly
                                                                           // draw
    private final int         BUFFER           = 15;                       // this
                                                                           // is
                                                                           // the
                                                                           // width
                                                                           // of
                                                                           ...
                                                                           // fixed-position
                                                                           // thing
    private int               highestSurfaceElev, lowestSurfaceElev,
            surfaceElevRange = -1;                                         // the
                                                                           // highest
                                                                           // and
                                                                           // lowest
                                                                           // elevations
    ...
    public Color              brighterGold     = new Color(218, 167, 0);   // biddable
                                                                           // 385
    public Color              darkestGold      = new Color(102, 79, 0);
    public Color              lightGreen       = new Color(177, 250, 162);
    public Color              brightGreen      = new Color(66, 184, 66);   // owned
                                                                           // by
                                                                           // me
    public Color              red              = new Color(184, 66, 66);   // owned
                                                                           // by
                                                                           // someone
                                                                           // else
    public Color              blue             = new Color(0, 153, 218);   // selected
                                                                           // for
                                                                           // seismic
    Color                     paleBlue         = new Color(109, 186, 218); // ghost
                                                                           // line
                                                                           // color
    ...
    public void initilize() {
        ...
        for (int i = 0; i < grid.getNumLayers(); i++) {
            try {
                ...
                rocksMGS[i] = ImageIO.read(getClass()
                        .getResource(grid.getRockArray()[i].imagePath()));
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    public void fillSquare(Graphics g, Color color, Point p) {
        ...
        if (surfaceElevRange == -1) { // This will occur twice when a session
            ...
            try {
                highestSurfaceElev =
                        request.operator.getGrid().getHighestSurfaceElev();
                lowestSurfaceElev =
                        request.operator.getGrid().getLowestSurfaceElev();
    ...
    public void drawRocks(Graphics g, Point p) {
        if (drawRocks) {
            g.drawImage(rocksMGS[request.operator.getGrid().getSurfaceLayer(p)],
                    p.x * squareSize + BUFFER, p.y * squareSize + BUFFER,
                ...
                if (grid.exists(p)) {
                    ...
                    for (int ot : oilTemp) {
                        if (ot > 0) {
                            ...
                            int imgSize = (int) Math
                                    .round((25.0 / 45.0 * squareSize));
                            g.drawImage(img,
                                    x * squareSize + 16 + squareSize / 2
                                            - imgSize / 2,
                    ...
                    for (int gt : gasTemp) {
                        if (gt > 0) {
                            ...
                            int imgSize =
                                    (int) Math.round((25.0 / 45.0 * squareSize))
                                            + 4;
                            g.drawImage(img,
                                    x * squareSize + 16 + squareSize / 2
                                            - imgSize / 2,
