...
public class Bid extends Action implements Comparable<Bid>{
	 ...
	 * @param owner The Operator who submitted the bid. 
	 * @param bidAmount The amount of the bid.
	 * @param land The location of the bid.
	 */
	 ...
	 * @param operator Owner of the bid.
	 * @param socketIn Socket message received that contains Bid values.
	 */
	 ...
	 * @param socketIn Socket message received that contains all Bid data.
	 */
	 ...
	 * @param socket The String array containing the data for the Bid.
	 */
	 ...
	 * @param bankLimit Current bank account value of
	 * operator that must be higher than amount of bid.
	 * @param maxP Point of the Bid on the Grid.
	...
	public boolean checkValid(int bankLimit, Point maxP) {
			if(owner.getGrid().getOwner(landP) == null ||
				...
				//if bidAmount is not limited by bank amount and is greater than min
				if(bankLimit >= bidAmount && bidAmount>=getMinBid()) {
					//if landP is inside of maxP and greater than 0
					if(landP.x < maxP.x && landP.y < maxP.y &&
							landP.x >= 0 && landP.y >= 0) {
						return true;
					}
				}
...
/*	public int getAmount() {
	 ...
	 * @param The bid to compare to this bid.
	...
	public String toSocket() {
		return "op#" + owner.getName() + "#price#" + bidAmount + "#y#" + landP.y + "#x#" + landP.x;
	}
