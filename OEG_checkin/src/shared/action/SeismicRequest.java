...
public class SeismicRequest extends Action {
	 ...
	 * @param owner Owner of the request, passed to the superclass
	 * @param startP Starting (upper leftmost) coordinate point.
	 * @param endP Ending (lower rightmost) coordinate point.s
	 */
	 ...
	 * @param operator Owner of the request.
	 * @param socketIn Socket message received
	 * that contains SeismicRequest values.
	 */
	...
	public boolean checkValid(Point maxP) {
		...
		if(owner.getBank().getAdjustedBalance() >= getCost()) {
			...
			if(startP.x >= 0 && endP.x >= 0 && startP.x < maxP.x && endP.x < maxP.x) {
				//if start and end y values are in bounds
				if(startP.y >= 0 && endP.y >= 0 && startP.y < maxP.y && endP.y < maxP.y) {
					...
					if(((startP.x - endP.x) == 0) || ((startP.y - endP.y) == 0)) {
						...
						if((startP.x <= endP.x||startP.x>endP.x) && (startP.y <= endP.y||startP.y>endP.y)) {
							if(startP.x>endP.x){
								int tempX= endP.x;
								endP.x=startP.x;
								startP.x=tempX;
					...
					}else if(Math.abs(startP.x-endP.x)==Math.abs(startP.x-endP.x)){
						return true;
					}
				}
			}
	...
	public String toString() {
		...
		return "Seismic Request -- Begin Cell: " +"(" + startP.x + ", " + startP.y + ")" +
					" - End Cell: " +"(" + endP.x + ", " + endP.y + ")\n    "
					+ "Cost: " + output + " - "
					+ super.toString();
	}
	
	/**formats the bid for sending over a socket.
	 * @return returns the bid as a string formatted for transfer over the socket
	 */
	public String toSocket() {
		return "sx#" + startP.x + "#sy#" +
				startP.y + "#ex#" + endP.x + "#ey#" + endP.y;
	}
	 ...
	 * Cost includes initial setup/mobilization cost plus
	 * additional cost based on length of survey line per grid cell
	 * @return Cost in $
	 */
	...
	public Point[] getPoints() {
		...
		if(Math.abs(startP.x - endP.x) == Math.abs(startP.y - endP.y)){
			...
			if (endP.x < startP.x){
				if(endP.y < startP.y){//
					System.out.print("startx"+startP.x+"  starty"+startP.y+"   endx"+endP.x+"   endy"+endP.y+"\n");
	...
	/**Checks to see if this SeismicRequest is equivalent to another Action, namely a SeismicRequest.
	 * @param other The action to compare to this SeismicRequest 
