...
public class Drill extends Action {
	 ...
	 * @param owner is the owner of the drill
	 * @param the point to drill.
	 */
	 ...
	 * @param owner, the owner of the drill.
	 * @param socketIn, the socket message
	 */
...
/*	public Drill(String socketIn) {
	 ...
	 * @param The String array containing the coordinates of the drill.
	 */
	 ...
	 * @param p the point of the Drill.
				...
				if(owner.getGrid().getOwner(p) != null)
					if(owner.getGrid().getOwner(p).equals(owner.getName())) {
						...
						if(!owner.getGrid().isDrilled(p)) {
							...
							if(owner.getGrid().isDrilled(p) == false)
								return true;
						}
					}
			}
	...
	/**Returns a '#' separated String containing information on oil and gas at this drill point.
	 * @param grid the grid to check for oil and gas.
	...
	public String toString() {
		...
		return "Drill -- Cost: " + output + " - Land: " +"(" + landP.x + ", " + landP.y + ") - "
				+ super.toString();
	}
}
