...
public class Messages extends Thread {
	...
	protected enum Prefix {
		...
		REMOVESEISMIC, REMOVEDRILL, BIDQUEUE, SEISMICQUEUE, DRILLQUEUE, LANDOWNER,
		ROCK, DRILLCOST,ENDGAME, WARNING, REGISTER, ENDROUND
	}

	/** Constructor initializes the socket that is used to send messages over the
	 * network.
	 * @param sock The socket that this object is wrapping. */
	 ...
	 * @param sock The socket being wrapped. */
	 ...
	 * @param child The child of Messages. Either ServerMessages or
	 * ClientMessages */
		...
		try {
			...
			if (ioe.getMessage().equals("Connection reset")) {
				...
				if (operator instanceof ServerOperator) {
					...
					//java.lang.ClassCastException: server.ServerMessages cannot be cast to client.ClientMessages
	 ...
	 * @param in The incoming string with both Prefix and message. Should be in
	...
	/** Parse abstract method to be extended by subclasses.
	 * Called by parse(String), after it determines what type of message the string is.
	 * @param in The string coming in from the socket to be parsed.
	 * @param pr The type of message being handled.*/
	 ...
	 * @param p The Prefix enumerated type of the message being sent.
	 * @param message The contents of the message being sent. */
	public void sendMessage(Prefix p, String message) {
		if (operator != null)
			System.out.println("OUT: " + operator.getName() + ": " + p.name() + ":" + message);
		if(sockOut != null)
		{
	 ...
	 * @param operator The operator object for this socket. */
