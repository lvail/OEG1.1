...
public class ServerOperator extends Operator {
	 ...
	 * @param d The director that is creating this object.
	 * @param name The name of the Operator.
	 * @param client The socket that is used by the Operator.
	 */
	public ServerOperator(Director d, String name, String password, ServerMessages client) {
		super(name, client);
		this.password = password;
		history = new LinkedList<Action>();
		client.sendBankBalance(bank.getStart());
		opGrid = new Grid(d.getGrid().getDimensions(), d
				.getGrid().getNumLayers());
	}

	/** Updates the Server Side bank with the winning bid cost.
	 * Notifys the client that they won the bid.
	 * Bid is placed in the opGrid elsewhere through setOwnership(Bid).
	 * @param won is the winning bid */
	public void wonBid(Bid won) {
		((ServerMessages) socket).wonBid(won.toSocket());
		bank.updateFunds(0 - won.getCost());
	}

	/** Notify the client that they lost the bid.
	 * Bid is placed in the opGrid elsewhere through setOwnership(Bid).
	 * @param won The bid that won the auction.
	 */
	...
	/** Send seismic information at a particular cell to the client.
	 * Seismic data is fudged. 
	 * @param p The point where seismic request was made.
	 * @param layer The layer information as an Integer array. */
	...
	/** Send drill information at a particular cell to the client.
	 * This includes gas and oil rates that are being returned by
	 * the results of the drilling. 
	 * @param p The point where the drill occurred.
	 * @param drillSocket The drill request formatted as a socket string.
	 * @param gas The gas results as an array.
	 * @param oil The oil results as an array.
	 * @param gasSocket The gas results as a socket string.
	 * @param oilSocket The oil results as a socket string.
	 * @param rockSocket The rock type results as a socket string.
	 * @param rock The rock type result as an array.
	 */
	public void sendDrillStuffs(Point p, String drillSocket, Integer[] gas,
			Integer[] oil, String gasSocket, String oilSocket,
			String rockSocket, String[] rock) {
	...
	public void sendEndGame(ServerOperator winner){
		...
		String outmessage=winner.teamName+" is the winner with "+output+" in their bank.";
		((ServerMessages) socket).sendEndGame(outmessage);
	}
		
	/** Add an item to the history log.
	 * @param s The Action to be added to the log.
	 */
	...
	/** Send end of round information to the client, including the new
	 * round number and current bankAccount information after action
	 * queues were processed. Also resets the action queues.
	 * @param timeRemaining Current time in the new countdown timer.
	 * @param roundLength The length of a round.
	 * @param curRound The current round number.
	 */
	public void endRound(long timeRemaining, long roundLength, int curRound, int numRounds) {
		Event endRound;
	...
	/** Set the ownership of a piece of land according to the winning bid that is passed.
	 * @param w The winning bid that contains the new location and owner of the piece of land.
	 */
	...
	/** Add to the income of this operator according to the gas and oil rates that are passed in
	 * @param gasRate The gas rate in num barrels per day.
	 * @param oilRate The oil rate in 
	 */
	public void addIncome(int gasRate, int oilRate) {
		//rate of production for a piece of land * income in $ per barrel * # of days per round
		bank.adjustIncome(gasRate * Action.gasRate * Director.timeStep);
		bank.adjustIncome(oilRate * Action.oilRate * Director.timeStep);		
	}
	
	/** Change the Team name
	 * @param newname The String reprenting the team's name
	 */
