...
public class Director {
	 ...
	 * input file.
	 * @param manage The GUI that the user of the server interacts with to view
	 * game information.
	 * @param xmlFile The XML file that is used to fill all OEG game parameters. */
	 ...
	 * @param xmlFile Input XML game file.
	...
	private Grid loadData(File xmlFile) {
		...
		try {
			...
			// in newGrid.
			ParseSimulationXML handler = new ParseSimulationXML(xmlFile, newGrid);
	 ...
	 * @param client The socket of the new client.
	 * @param name The team name of the new client. */
	public void newClient(ServerMessages client, boolean register, String name, String password) {
		if (register && manage.registerDisabled)
		{
			client.sendWarning("Registration has been disabled by the Instructor.");
			return;
		}
		...
		if (exists == -1) {
			if (!register)
			{
				client.sendWarning("Invalid team name, please use register if you wish to create a team.");
				return;
			}
			// create a new Operator with team name and socket
			addOp = operators[numOperators] = new ServerOperator(this, name, password,
					client);
			...
			operators[exists].closeSocket();
			// add the new socket to existing operator object
			operators[exists].setSocket(client);
			// send acknowledgment
			operators[exists].sendInfo("Welcome Back!");
			// send the operator current game information
			sendGameInfo(client);
			operators[exists].sendExistingData();
		}
	 ...
	 * information.
	 * @param client The client socket to send the info to. */
	...
	/**To be called by a director or other unexpexcted source-- has ALL end-round logic*/
	protected void unscheduledEndRound(){
			System.out.println("UNSCHEDULED End Round called");
			this.setupAuction();
			//System.out.println("Auction Set Up");
			this.processSeismicRequests();
			//System.out.println("Seismic requests processed");
			this.processDrillRequests();
			//System.out.println("Drill requests processed");
			...
			System.out.println("Timer set up");
			...
			for (int i = 0; i < numOperators; i++) {
				...
				operators[i].endRound(roundLength, roundLength,
						curRound, numRounds);
				System.out.printf("Player %d: %s Notified of End of Round\n", i, operators[i].toString());
			}
	...
	public void setupAuction() {
		...
		// fill the array
		for (int i = 0; i < numOperators; i++) {
			list[i] = operators[i].getBidQueue();
		}
		// start the auction
	...
	public void processDrillRequests() {
		...
		for (int i = 0; i < numOperators; i++) {
			...
			for (Drill d : drill) {
				...
				operators[i].sendDrillStuffs(d.toSocket(), grid.cellToSocket(p));
						/*d.toSocket(),
						grid.getGasArray(p), grid.getOilArray(p),
	 ...
	 * @param length is the amount of time in MS that each round is */
	public static void setRoundTime(long length) {
		roundLength = length;
	}

	/** Set how long the game will last, in number of rounds.
	 * @param number is the amount of rounds in the simulation */
	 ...
	 * @param amount is the amount in days that each round represents */
	...
	/**
	 * This method is called when the game is ended, and loops through each of the clients and sets the game's winner
	 */
