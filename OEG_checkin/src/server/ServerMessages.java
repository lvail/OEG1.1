...
public class ServerMessages extends Messages {
	 ...
	 * @param inDirector Director that incoming messages from the client will be
	 * sent to.
	 ...
	 * @param in The message to be passed.
	 * @param pr The prefix identifying the message. */
	public void parse(String in, Prefix pr) {
		...
		switch (pr) {
			...
			case REGISTER:
				...
				director.newClient(this, pr == Prefix.REGISTER, parts[0], parts[1]);
				this.name = parts[0];
				break;
			case TEST:
			...
			case REMOVEDRILL:
				...
				director.manage.refreshInfo();
				break;
				//TODO Don't think needed. If any problems with Gas or Oil, uncomment.
			/*case OIL:
				((ServerOperator) operator).newDrillRequest(in);
				break;
			case GAS:
	 ...
	 * @param milliseconds Time remaining in the current countdown.
	 * @param roundLength Length of the countdown/each round.
	 * @param roundNum The current round number. */
	public void startTimer(long milliseconds, long roundLength, int roundNum, int numRounds) {
		sendMessage(Prefix.STARTTIME, milliseconds + "#" + roundLength + "#"
				+ roundNum + "#" + numRounds);
	}

	/** Signal a new round start, and send the current bank balance to the client.
	 * @param balance The amount in $ of the operator's current bank balance. */
	 ...
	 * @param amount The amount in $ of the operator's current bank balance. */
	 ...
	 * @param warning Message to be sent.
	 */
	 ...
	 * @param xLimit Number of cells horizontally.
	 * @param yLimit Number of cells vertically.
	 * @param numLayers Depth of cells.
	 */
	public void sendGridLimits(int xLimit, int yLimit, int numLayers) {
		sendMessage(Prefix.GRIDLIMITS, xLimit + "#" + yLimit + "#" + numLayers);

	}

	/** Send cost of seismic line requests.
	 * @param in String representing cost.
	 */
	 ...
	 * @param layers String representing rocktype array.
	 */
	 ...
	 * @param gas String representing gas array.
	 */
	 ...
	 * @param oil String representing oil array.
	 */
	 ...
	 * @param c String representing the entire cell */
	public void sendCell(String c) {
		sendMessage(Prefix.CELL, c);
	}

	/** Send the results of a drill request.
	 * @param drillSocket String representing the drilling result.
	 */
	 ...
	 * @param socket The bid queue as a string.
	 */
	 ...
	 * @param socket The seismic queue as a string.
	 */
	 ...
	 * @param socket The drill queue as a string.
	 */
