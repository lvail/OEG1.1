...
public class ParseSimulationXML extends DefaultHandler {
    ...
    public enum Element {
        ...
        OIL, GAS, ROCKTYPE, BOTTOM, SIMTIMESTEPS, NUMSTEPS, STEPTIME, SIMMONEY,
        SEISMICSETUP, SEISMICLINEAR, STARTCASH, DRILLCOST, MINLEASECOST,
        OILINCOME, GASINCOME, FUDGE, REMOVEPERCENT
    }
     ...
     * @param xmlFile
     * @param grid
     *            The Grid object that information in the XML is being added to.
     */
     ...
     * 
     * @param qName
     *            gives the name of the element
     * @param attributes
     *            gives any attribute values tied in the element start tag.
     */
    @Override
    public void startElement(String uri, String localName, String qName,
        ...
        if (elementMap.containsKey(qName.toUpperCase())) {
            ...
            if (stack.peek() == parentMap.get(name))
                ...
                throw new SAXException(
                        "Element type " + qName + " has incorrect nesting");
            }
        } else {
            throw new SAXException(
                    "Unknown Start Element Type: " + qName + ".");
        }
        ...
        switch (stack.peek()) {
            case SIMDOMAIN:
                ...
                break;
            case SIMTIMESTEPS:
                Director.setTimeStep(
                        Integer.parseInt(attributes.getValue("simsteptime")));
                break;
            case LAYER:
                if (!gridStart) {
            ...
            case TOP:
                ...
                csvFile = new File(xmlFile.getParentFile().getAbsolutePath(),
                        fileName);
        ...
        if (!stack.empty()) {
            switch (stack.peek()) {
                ...
                case YDIM:
                    ...
                    break;
                case FUDGE:
                    grid.setFudgePercent(
                            Integer.parseInt(new String(ch, start, length)));
                    break;
                case REMOVEPERCENT:
                    grid.setRemovePrecent(
                            Integer.parseInt(new String(ch, start, length)));
                    break;
                case ROCKTYPE:
                    grid.addRock(
                            new LithologicType(Integer
                ...
                case STEPTIME:
                    ...
                    break;
                case NUMSTEPS:
                    Director.setNumRounds(
                            Integer.parseInt(new String(ch, start, length)));
                    break;
                case SEISMICSETUP:
                    Action.setSeismicSetup(
                            Integer.parseInt(new String(ch, start, length)));
                    break;
                case SEISMICLINEAR:
                    Action.setSeismicRate(
                            Integer.parseInt(new String(ch, start, length)));
                    break;
                case STARTCASH:
                    BankAccount.setStart(
                            Integer.parseInt(new String(ch, start, length)));
                    break;
                case DRILLCOST:
                    Action.setDrillCost(
    ...
    private class SIMDOMAIN extends Hierarchy {
        {
            super.children = new Element[] { Element.FUDGE,
                    Element.REMOVEPERCENT, Element.XDIM, Element.YDIM,
    ...
    private class SIMTIMESTEPS extends Hierarchy {
        {
            super.children =
                    new Element[] { Element.NUMSTEPS, Element.STEPTIME };
    ...
    private class SIMMONEY extends Hierarchy {
        {
            super.children = new Element[] { Element.SEISMICSETUP,
                    Element.SEISMICLINEAR, Element.STARTCASH, Element.DRILLCOST,
                    Element.MINLEASECOST, Element.OILINCOME,
                    Element.GASINCOME };
