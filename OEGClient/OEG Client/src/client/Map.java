...
class Map extends JPanel implements ActionListener, MouseListener, MouseMotionListener {
	...
	private int size, height, width; // size height and width variables
	...
	public void initilize() {
		...
		squareSize = height / size;// (h - 30)/s;

		// set minimum square size
		if (squareSize < minSize) {
			squareSize = minSize;
			height = width = squareSize * size;
		}
		...
		xMinCords = new int[size + 1];
		yMinCords = new int[size + 1];
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i <= size; i++) {
			g.drawLine(x, y, x, (BUFFER + (size * squareSize)));
			xMinCords[i] = x;
			x += squareSize;
		}
		...
		// draw vertical lines
		for (int i = 0; i <= size; i++) {
			g.drawLine(x, y, (BUFFER + (size * squareSize)), y);
			yMinCords[i] = y;
			y += squareSize;
		}
	...
	public void drawCompass(Graphics g) {
		...
		g.drawString("N", ((width + BUFFER * 2) / 2) - f.charWidth('N') / 2, BUFFER / 2 + f.getHeight() / 2 - 2);
		g.drawString("S", ((width + BUFFER * 2) / 2) - f.charWidth('S') / 2,
				(height + BUFFER * 2) - BUFFER / 2 + f.getHeight() / 2 - 2);
		g.drawString("E", (width + BUFFER * 2) - BUFFER / 2 - f.charWidth('E') / 2,
				((height + BUFFER * 2) / 2) + f.getHeight() / 2);
		g.drawString("W", BUFFER / 2 - f.charWidth('W') / 2 + 1, ((height + BUFFER * 2) / 2) + f.getHeight() / 2);
	}
	...
	public void onMouseClick(MouseEvent event) {
		...
		if (!(event.getX() > (squareSize * size + BUFFER) || event.getY() > squareSize * size + BUFFER
	...
	public void paintColorsMode(Graphics g, int x, int y) {
		...
		if (rbs.equals("B")) { // If the bid radio box is selected
			...
			doDrill(p);
		} else if (rbs.equals("S")) { // if the seismic line radio box is
										// selected
			...
			if (sMode == 0) {
				...
				String cost = request.operator.getCostSeismic(startP.x, startP.y, endP.x, endP.y);
	...
	public void fillSquare(Graphics g, Color color, Point p) {
		...
		if (surfaceElevRange == -1) { // This will occur twice when a session
										// begins; once, because there is no
										// data,
			...
			try {
				...
				surfaceElevRange = highestSurfaceElev - lowestSurfaceElev;
												...
												// error, when map is created
												// but
												// cell data has not yet been
												// sent.
				return;
			}
		}
	...
	public void drawRocks(Graphics g, Point p) {
		if (drawRocks) {
			g.drawImage(rocksMGS[request.operator.getGrid().getSurfaceLayer(p)], p.x * squareSize + BUFFER,
					p.y * squareSize + BUFFER, squareSize, squareSize, null);
		}
	}
	...
	public void seismicColor(Graphics g, Point startP, Point endP) {
		...
		if (startP.equals(endP)) {
			...
			for (int i = (x1 + 1); i < size; i++) {// horizontal right ghost
				selectCell(g, paleBlue, new Point(i, y1));
			}
			for (int i = 0; i < y1; i++) {// vertical up ghost
				selectCell(g, paleBlue, new Point(x1, i));
			}
			for (int i = (y1 + 1); i < size; i++) {// vertical down ghost
				selectCell(g, paleBlue, new Point(x1, i));
			}
			int j = 0;
			...
			for (int i = (x1 + 1); i <= size; i++) {// diagonal up/right ghost
				j = i - x1;
				j = y1 - j;
				if (i < size && j >= 0) {
					selectCell(g, paleBlue, new Point(i, j));
			...
			for (int i = (x1 + 1); i <= size; i++) {// diagonal down/right ghost
				j = i - x1;
				j = y1 + j;
				if (i < size && j < size) {
					selectCell(g, paleBlue, new Point(i, j));
				...
				if (grid.exists(p)) {
					...
					if (owner != null) {
						if (owner.equals(player)) {
							Color green = new Color(46, 184, 0);
							fillSquare(g, green, p);
						} else {
							Color grey = new Color(240, 240, 240);
							// fillSquare(g, grey,p.x,p.y);
						}
						if (grid.isDrilled(p)) {// light green
							Color lightGreen = new Color(177, 250, 162);
							fillSquare(g, lightGreen, p);
						}
					} else {
						Color grey = new Color(240, 240, 240);
						// fillSquare(g, grey,p.x,p.y);
	...
	public void setCellInfo(int x, int y) {
		...
		if (grid.exists(p)) {
			...
			while ((grid.layerExists(p, depth) == false) && depth < grid.getNumLayers())
				depth++;
				...
				if (grid.exists(p)) {
					...
					for (int ot : oilTemp) {
						if (ot > 0) {
							...
							g.drawImage(img, x * squareSize + 16 + squareSize / 2 - imgSize / 2,
									y * squareSize + 16 + squareSize / 2 - imgSize / 2, imgSize, imgSize, null);
