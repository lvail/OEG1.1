...
class Map extends JPanel implements ActionListener, MouseListener, MouseMotionListener
{
	...
	private final int BUFFER = 15; //this is the width of the edge of the map and always included when drawing a fixed-position thing
	private int highestSurfaceElev, lowestSurfaceElev, surfaceElevRange = -1; //the highest and lowest elevations on the map's surface, and the difference between
	 ...
	 * @param request The GUI object
	 * @param p The size of the grid, represented as a Point.
	 */
	...
	public void initilize(){
		...
		//Initialize the layer images so they are retrieved once (not for every cell)
		...
		for(int i = 0; i < grid.getNumLayers(); i++){
			try {
				...
				rocksMGS[i] = ImageIO.read(getClass().getResource(grid.getRockArray()[i].imagePath()));
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
	...
	public void drawCompass(Graphics g) {
		...
		g.drawString("N", ((w+BUFFER*2)/2)-f.charWidth('N')/2, BUFFER/2+f.getHeight()/2-2);
		g.drawString("S", ((w+BUFFER*2)/2)-f.charWidth('S')/2, (h+BUFFER*2)-BUFFER/2+f.getHeight()/2-2);
		g.drawString("E", (w+BUFFER*2)-BUFFER/2-f.charWidth('E')/2, ((h+BUFFER*2)/2)+f.getHeight()/2);
		g.drawString("W", BUFFER/2-f.charWidth('W')/2+1, ((h+BUFFER*2)/2)+f.getHeight()/2);
	}
	...
	public void onMouseClick(MouseEvent event) {
		...
		alternate the sMode in between start and end */
	...
	public void fillSquare(Graphics g, Color color, Point p)
	{
		int elev;
		double gradient;
		if(surfaceElevRange == -1){ //This will occur twice when a session begins; once, because there is no data, and once when there finally is data.
			try{
				highestSurfaceElev = request.operator.getGrid().getHighestSurfaceElev();
				lowestSurfaceElev = request.operator.getGrid().getLowestSurfaceElev();
				surfaceElevRange = highestSurfaceElev-lowestSurfaceElev;
			}
			catch(NullPointerException e){ //This will occur on the first error, when map is created but cell data has not yet been sent.
				return;
			}
		}
		...
		g.fillRect(xMinCords[p.x]+1, yMinCords[p.y]+1, squareSize-1, squareSize-1);
	...
	public void selectCell(Graphics g, Color c, Point p)
	{
		if(c==null)
			...
			g.setColor(c);
		g.drawRect(p.x*squareSize+BUFFER+1, p.y*squareSize+BUFFER+1, squareSize-2, squareSize-2);
	...
	public void drawRocks(Graphics g, Point p){
		if(drawRocks){
			g.drawImage(rocksMGS[request.operator.getGrid().getSurfaceLayer(p)], p.x*squareSize+BUFFER,  p.y*squareSize+BUFFER, squareSize, squareSize, null);
		}
		}
				...
				if (grid.exists(p)){
					...
					for(int ot : oilTemp) {
						if (ot > 0){
							...
							try {
								img = ImageIO.read(getClass().getResource("/client/images/oil_well_small.gif"));

								//img = ImageIO.read(new File("C:/Users/jobotte1/Desktop/oil_small.gif"));

							} catch (IOException e){
								e.printStackTrace();
							}
							int imgSize = (int)Math.round((25.0/45.0*(double)squareSize));
							g.drawImage(img, x*squareSize+16+squareSize/2-imgSize/2,  y*squareSize+16+squareSize/2-imgSize/2, imgSize, imgSize, null);
					...
					for(int gt : gasTemp) {
						if (gt > 0){
							try {
								img = ImageIO.read(getClass().getResource("/client/images/gas_well_small.gif"));
							...
							int imgSize = (int)Math.round((25.0/45.0*(double)squareSize))+4;
							g.drawImage(img, x*squareSize+16+squareSize/2-imgSize/2,  y*squareSize+16+squareSize/2-imgSize/2, imgSize, imgSize, null);
					...
					for(Bid b : bidQ) {
						Point bidPoint = b.getPoint();
						try {
							img = ImageIO.read(getClass().getResource("/client/images/dollar.gif"));      	
						} catch (IOException e){
							e.printStackTrace();
						}
						...
						int imgSize = (int)Math.round((25.0/45.0*(double)squareSize))+4;
						g.drawImage(img, bidPoint.x*squareSize+16+squareSize/2-imgSize/2,  bidPoint.y*squareSize+16+squareSize/2-imgSize/2,  null);
					}*/
